...
public class allpost : MonoBehaviour {
	...
	{	
		...
		Hot.AddComponent<allpost_pop>();
		
		allpost_pop PostType = Hot.GetComponent<allpost_pop> ();
		PostType.city = city;
		...
		Loom.RunAsync (() => {
			...
			var queryC = ParseObject.GetQuery ("POST").WhereDoesNotExist("Post_Geo").WhereEqualTo("Location",city).OrderByDescending ("createdAt").Limit(5);
