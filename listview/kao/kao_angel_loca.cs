...
public class kao_angel_loca : MonoBehaviour {
	...
	{	
		...
		Loom.RunAsync (() => {
			...
			var query = ParseObject.GetQuery ("POST").WhereEqualTo("foo","angel").WhereEqualTo("Location","kaoshiung").WhereExists("post_geo").OrderByDescending ("createdAt").Limit(limit);
